...
class CreateRelawanTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('relawan', function (Blueprint $table) {
            ...
            $table->foreign('id_profesi')->references('id')->on('ref_profesi')->onDelete('cascade')->onUpdate('cascade');
            $table->integer('id_jk')->unsigned()->nullable();
            $table->foreign('id_jk')->references('id')->on('ref_vendor_saving')->onDelete('cascade')->onUpdate('cascade');
